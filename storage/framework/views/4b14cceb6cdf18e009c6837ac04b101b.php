...
<?php echo view_render_event('bagisto.admin.catalog.product.edit_form_accordian.grouped_products.before', ['product' => $product]); ?>


<accordian title="<?php echo e(__('admin::app.catalog.products.grouped-products')); ?>" :active="true">
    <div slot="body">

        <?php echo view_render_event('bagisto.admin.catalog.product.edit_form_accordian.grouped_products.controls.before', ['product' => $product]); ?>


        <grouped-product-list></grouped-product-list>

        <?php echo view_render_event('bagisto.admin.catalog.product.edit_form_accordian.grouped_products.controls.after', ['product' => $product]); ?>


    </div>
</accordian>

<?php echo view_render_event('bagisto.admin.catalog.product.edit_form_accordian.grouped_products.after', ['product' => $product]); ?>
        ...
        <tr>
            ...
            <td>
                <div class="control-group" :class="[errors.has(inputName + '[qty]') ? 'has-error' : '']">
                    <input type="number" v-validate="`required|min_value:0|max_value:${available_qty}`" :name="[inputName + '[qty]']" v-model="groupedProduct.qty" class="control" data-vv-as="&quot;<?php echo e(__('admin::app.catalog.products.qty')); ?>&quot;"/>
            ...
            <td>
                <div class="control-group" :class="[errors.has(inputName + '[sort_order]') ? 'has-error' : '']">
                    <input type="number" v-validate="'required|min_value:0'" :name="[inputName + '[sort_order]']" v-model="groupedProduct.sort_order" class="control" data-vv-as="&quot;<?php echo e(__('admin::app.catalog.products.sort-order')); ?>&quot;"/>
        ...
        Vue.component('grouped-product-list', {
            ...
            data: function() {
                return {
                    ...
                    grouped_products: <?php echo json_encode($product->grouped_products()->with('associated_product.inventory_indices')->get(), 15, 512) ?>
                }
            },
...
<?php $__env->stopPush(); ?><?php /**PATH /home/ubuntu/volantiScottsdale/packages/Webkul/Admin/src/Resources/views/catalog/products/accordians/grouped-products.blade.php ENDPATH**/ ?>